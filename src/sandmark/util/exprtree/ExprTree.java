...

{
	...
	sandmark.util.exprtree.Node gn;
	...
	/**
       ...
       @param c - the ConstantPoolGen created from the class that this method is in
       @param mycfg - the controlflowgraph of that method

     */

	public ExprTree(sandmark.program.Method method,
...
private void initialize()
{
    ...
    {
	gn= new sandmark.util.exprtree.Node();
	ni= new NodeInfo();
	ni.setIH(ih[i]);
	IToN.put(ih[i],gn);
	NToI.put(gn,ih[i]);
...
{		org.apache.bcel.generic.InstructionHandle tempih;
		...
		sandmark.util.exprtree.Node dummynode;
		NodeInfo dummyInfo;
		...
		for(int j=i-1;j>=0;j--)
		{
				         sd=cn.getStackAt(j);
					         ...
					         if(ilist.contains(tempih)&& tempih!=ihandle)
					         {
						 	 ...
						 	 {
						 	 	dummynode=new sandmark.util.exprtree.Node();
...
private void doBlock()
{
	...
	sandmark.util.exprtree.Node tempn;
	...
	for(int parse=0;parse<ilist.size();parse++)
	{
		...
		gn=(sandmark.util.exprtree.Node )IToN.get(ihandle);
...
public String toString()
{
	...
	{	Grlist=(java.util.ArrayList) BToL.get(cfg.nodes().get(j));
		...
		for(int i=0;i<getRootNodes(myGr).size();i++)
			{
					S+=doPrintPostOrder((sandmark.util.exprtree.Node)
...
private String doPrintPostOrder(sandmark.util.exprtree.Node myGn
										,int level,int up)
{	String S="";
	   ...
	   while (succIter.hasNext()) {
	      sandmark.util.exprtree.Node  bb =
		 (sandmark.util.exprtree.Node )succIter.next();
	...
	if(myGn.graph().numSuccs(myGn)>=1) {
	   java.util.Iterator succIter = myGn.graph().succs(myGn);
	   sandmark.util.exprtree.Node succ =
	      (sandmark.util.exprtree.Node)succIter.next();
...
class GraphComparator implements java.util.Comparator{
	...
	{nc=new NodeComparator(tt);
	 ...
	 public int compare(Object gr1, Object gr2)
	 {
	   	 ...
	   	 NodeInfo tempni;
	   	 sandmark.util.exprtree.Node tempnode=null;
	   	 sandmark.util.newgraph.MutableGraph myGr
...
{	java.util.ArrayList templist = new java.util.ArrayList();
	...
	java.util.Iterator nodeIter = gr.nodes();
		while (nodeIter.hasNext()) {
		  sandmark.util.exprtree.Node tempnode =
		      (sandmark.util.exprtree.Node)nodeIter.next();
		ls.add(tempnode);
	}
	...
	while (nodeIter.hasNext()) {
	  sandmark.util.exprtree.Node tempnode =
	   ...
	   while (predIter.hasNext()) {
	      sandmark.util.exprtree.Node debugnode =
		 (sandmark.util.exprtree.Node)predIter.next();
...
/**
       Returns the sandmark.util.Exprtree.NodeInfo associated with a sandmark.util.exprtree.Node .
       @param tempgn a node in the expression tree whose corresponding information is desired
*/
public sandmark.util.exprtree.NodeInfo nodeToInfo(sandmark.util.exprtree.Node  tempgn)
{
 return (sandmark.util.exprtree.NodeInfo) NToInfo.get(tempgn);

}

/* It returns the node associated with an instruction */
public sandmark.util.exprtree.Node  iToNode(org.apache.bcel.generic.InstructionHandle x)
{
	return  (sandmark.util.exprtree.Node )IToN.get(x);

}

/* It returns the instruction associated with a node */
public org.apache.bcel.generic.InstructionHandle nodeToI(sandmark.util.exprtree.Node  x)
{
